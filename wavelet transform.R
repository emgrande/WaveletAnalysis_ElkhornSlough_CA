...
#here I have added only one file to use as an example (data.csv)
...
wavelets <- function(df){
  ...
  data <- df
  #convert to date
  data$DateTime <- lubridate::mdy_hm(data$DateTime) #you might have to check your date time format if you get an error. ?lubridate
  
  #the data has already been cleaned by the author,
  #we can just run the continuous wavelet transform
  cwt <-biwavelet::wt(data, dt = 1)
  ...
  cwt1 <- biwavelet::wt(data1, dt = 1)
  sig_95 <- biwavelet::wt.sig(data1, sig.level = 0.95, scale = cwt1$scale, dt = 1, sigma2 = cwt1$sigma2)
  ...
  print("Plotting wavelet transform...")
  #plot the cwt
